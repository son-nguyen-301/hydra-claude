...
Use this checklist to evaluate and improve prompts before execution.

When hydra memory is available, prefer filling the **Context** and **Constraints**
sections from captured repo patterns (`.claude/memory/plugin/`) over asking the user
or inventing generic placeholders. Only ask the user about a gap when memory cannot
supply it and it is material to the task.
...
**Prefer memory:** if a captured topic file records the affected module, conventions,
or prior decisions, cite those facts here instead of asking.
...
**Prefer memory:** captured corrections ("never use X", "always do Y") are strong
sources for the constraints block.
